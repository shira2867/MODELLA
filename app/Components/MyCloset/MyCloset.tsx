...
// עובד אבל צריך ךסדר שיהיה זהה לקודם
...
//   let closetContent: React.ReactNode;
//   if (isLoading) {
//     closetContent = <div className={styles.loaderContainer}><div className={styles.dashedSpinner}></div></div>;
//   } else if (filteredClothes.length === 0) {
//     closetContent = <p className={styles.noClothes}>No items found.</p>;
//   } else {
//     closetContent = (
...
//                   if (isMobile) {
//                     showToast(
//                       "Cannot create a look on mobile. Please use a computer or tablet.",
//                       "info"
//                     );
//                   }
//                 }}
...
//   return <div className={styles.container}>{closetContent}</div>;
// };
...
const MyCloset: React.FC<MyClosetProps> = ({
  ...
  const filteredClothes = clothes.filter((item) => {
    ...
    if (
      inspirationColors.length > 0 &&
      (!item.colorName || !inspirationColors.includes(item.colorName))
    ) {
      return false;
    }
